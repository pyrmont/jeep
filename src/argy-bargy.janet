...
(defn- usage
  ...
  (when (info :about)
    (print)
    (print (indent-str (info :about) 0 0)))
  ...
  (when (info :rider)
    (print)
    (print (indent-str (info :rider) 0 0))))
...
(defn- conform-cmds
  ...
  (def pcmd (->> (get args 0) (string/split sep) last))
  (if has-subcommand?
...
(defn parse-args
  ...
  * `:rest` - Whether this rule should capture all following parameters. Only
  ...
  Once parsed, the return value is a struct with `:opts` and `:params` keys.
